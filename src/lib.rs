...
impl CronSchedule {
    ...
    pub fn next_time_from(&self, time: time::OffsetDateTime) -> time::OffsetDateTime {
        ...
        let result = loop {
            ...
            let (_year, month, day) = next.to_calendar_date();

            if let Err(idx) = self.month.binary_search(&Month::from_num_asserted(month as u8)) {
            ...
            if let Err(idx) = self.day_m.binary_search(&DayOfMonth::from_num_asserted(day)) {
                ...
                let date = match self.day_m.get(idx).and_then(|day| time::Date::from_calendar_date(next.year(), month, (*day).into()).ok()) {
                    Some(date) => date,
                    //If next allowed day doesn't fit the current month, then just switch to next month, unless it is last month
                    None if (month as u8) < Month::MAX => time::Date::from_calendar_date(next.year(), month.next(), 1).expect("Get next month date"),
            ...
            if let Err(idx) = self.day_w.binary_search(&Day::from_num_asserted(weekday_s)) {
                let date = match self.day_w.get(idx) {
                    Some(day_w) => match time::Date::from_calendar_date(next.year(), month, day + *day_w as u8 - weekday_s) {
                        //Day is on current week.
                        Ok(date) => date,
                        //Day is in next month so iterate onto next month (note weekday enum is in range 0..6)
                        Err(_) if (month as u8) < Month::MAX => time::Date::from_calendar_date(next.year(), month.next(), *day_w as u8 - weekday_s).expect("Get next month date"),
